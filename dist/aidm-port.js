...
(()=>{
  ...
  function renderLegacyRoutes(root,state){
    const message=document.createElement('p');message.className='small';message.textContent='The signed map will refresh after your next server turn.';root.append(message);
    for(const route of Array.isArray(state?.exits)?state.exits.slice(0,8):[])root.append(button(`Travel: ${clean(route)}`,()=>prepareTravel(route)));
  }
  ...
  function renderBattle(state){
    ...
    const info=document.createElement('p');info.className='small';info.textContent=`Round ${combat.round||1} · ${combat.turn==='hero'?'Your turn':'Enemy turn'}`;root.append(info);
    ...
    const hero=combat.actors?.hero,foe=combat.actors?.['enemy-1'],actors=document.createElement('p');actors.className='small';actors.textContent=`You: ${hero?.hp??state.hp}/${hero?.maxHp??state.maxHp} HP${foe?` · ${foe.name}: ${foe.hp}/${foe.maxHp} HP`:''}`;root.append(actors);
