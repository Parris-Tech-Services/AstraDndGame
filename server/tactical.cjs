...
function move(state,x,y,roll=randomInt){
  ...
  if(wasInReach&&enemy&&distanceFt(hero,enemy)>5&&enemy.reactionAvailable&&!hero.disengaged){
    ...
    if(hit){const damage=die(6,roll)+1;damageHero(next,damage);summary+=` ${enemy.name} used an opportunity attack and dealt ${damage} damage.`}
...
function dash(state){
  ...
  const hero=combat.actors.hero;if(hero.actionUsed)return refreshReachable(next,{ok:false,summary:'Your action is already used.'});
  hero.actionUsed=true;hero.dashed=true;const summary='You Dash, doubling your movement allowance this turn.';combat.log=[...(combat.log||[]),summary].slice(-30);return refreshReachable(next,{ok:true,summary});
}
function disengage(state){
  ...
  const hero=combat.actors.hero;if(hero.actionUsed)return refreshReachable(next,{ok:false,summary:'Your action is already used.'});
  hero.actionUsed=true;hero.disengaged=true;const summary='You Disengage; movement this turn will not trigger opportunity attacks.';combat.log=[...(combat.log||[]),summary].slice(-30);return refreshReachable(next,{ok:true,summary});
}
...
function attack(state,targetId='enemy-1',roll=randomInt){
  ...
  const hero=combat.actors.hero,target=combat.actors[targetId];if(!hero||!target||target.hp<=0)return refreshReachable(next,{ok:false,summary:'There is no valid target.'});
  if(hero.actionUsed)return refreshReachable(next,{ok:false,summary:'Your action is already used.'});
  ...
  combat.log=[...(combat.log||[]),summary].slice(-30);return refreshReachable(next,{ok:true,summary,roll:{die:attackDie,total,cover},targetHp:target.hp});
}
function enemyAttack(state,roll=randomInt){
  ...
  const damage=die(6,roll)+1;damageHero(state,damage);return `${enemy.name} hits for ${damage} damage.`;
}
