...
function upgrade(input){
  ...
  Object.assign(s,id);
  delete s.progressionEvent;
  if(typeof s.level!=='number')s.level=2;
...
function resolve(state,plan,roll=randomInt,action=''){
  ...
  if(resource==='secondWind'){s.secondWindReady=false;resolution.healing=Math.min(s.maxHp-s.hp,roll(1,11)+s.level);s.hp+=resolution.healing}
  if(plan.kind!=='none'){
    const a=roll(1,21),b=advantage!=='normal'?roll(1,21):null,die=b===null?a:advantage==='advantage'?Math.max(a,b):Math.min(a,b),mod=classes[s.cls].stats[plan.ability]+(proficient?rules.proficiencyBonus(s.level):0),dc=num(plan.dc,5,25,12),total=die+mod,critThresh=rules.criticalThreshold(s),success=plan.kind==='attack'?(die>=critThresh||(die!==1&&total>=dc)):total>=dc;
    resolution.roll={dice:b===null?[a]:[a,b],die,modifier:mod,total,dc,ability:plan.ability,advantage,success,critical:plan.kind==='attack'&&die>=critThresh};s.lastRoll=resolution.roll;
...
function apply(state,result,action,resolution){
  ...
  if(!resolution.blocked){
    ...
    const progression=rules.levelUp(s);
    if(progression){
      s.narrative+=`\n\n*** LEVEL UP! ***\nYou have reached Level ${progression.newLevel}! Maximum HP increased by ${progression.hpIncrease}.`;
      if(progression.unlocks.length>0)s.narrative+=` Unlocks: ${progression.unlocks.join(', ')}.`;
      s.progressionEvent=progression;
    }
    if(explicitRest){s.hp=rest==='long'?s.maxHp:Math.min(s.maxHp,s.hp+6);if(s.cls==='fighter')s.secondWindReady=true;if(rest==='long'&&s.cls==='wizard')s.slots=rules.maxWizardSlots(s.level);}
  }
