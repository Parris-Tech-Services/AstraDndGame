...
module.exports=async function handler(req,res){
  ...
  }catch(e){console.error('[astra tactical]',e instanceof Error?e.message:String(e));console.error('[astra-dnd telemetry]',JSON.stringify({route:'/api/tactical',op,status:500,latency:Date.now()-startTime}));return reply(res,500,{error:'The tactical action could not be resolved. Your previous save is unchanged.'})}
  const enriched=tactical.enrichSpatial(result.state);
  console.log('[astra-dnd telemetry]',JSON.stringify({route:'/api/tactical',op,status:result.ok===false?409:200,latency:Date.now()-startTime}));
