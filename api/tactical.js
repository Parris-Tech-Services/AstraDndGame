...
function keysFor(op){if(op==='start')return new Set([...baseKeys,'enemyName']);if(op==='move')return new Set([...baseKeys,'x','y']);if(op==='attack')return new Set([...baseKeys,'targetId']);return baseKeys}
function validate(body){if(typeof body.op!=='string'||!['start','move','dash','disengage','attack','end_turn','sync'].includes(body.op)||!http.onlyKeys(body,keysFor(body.op))||typeof body.save!=='string'||body.save.length>80000||!http.validRequestId(body.requestId))return null;if(body.op==='start'&&body.enemyName!==undefined&&(typeof body.enemyName!=='string'||body.enemyName.length>60))return null;if(body.op==='move'&&(!Number.isInteger(body.x)||!Number.isInteger(body.y)))return null;if(body.op==='attack'&&body.targetId!==undefined&&(typeof body.targetId!=='string'||body.targetId.length>80))return null;return body}
...
function execute(state,input){if(input.op==='start')return {state:tactical.startEncounter(state,input.enemyName),ok:true,summary:null};if(input.op==='move')return tactical.move(state,input.x,input.y);if(input.op==='dash')return tactical.dash(state);if(input.op==='disengage')return tactical.disengage(state);if(input.op==='attack')return tactical.attack(state,input.targetId||'enemy-1');if(input.op==='end_turn')return tactical.endTurn(state);return {state:tactical.enrichSpatial(state),ok:true,summary:'Spatial state refreshed.'}}
...
module.exports=async function handler(req,res){const secret=getSecret();if(req.method==='GET')return http.reply(res,200,{configured:!!secret,signingMode:secretPolicy.signingMode(env),mode:'tactical',features:['exploration-map','battle-grid','movement','dash','disengage','opportunity-attacks','range','line-of-sight','cover','turn-economy'],build:(env.VERCEL_GIT_COMMIT_SHA||'local').slice(0,12)});if(req.method!=='POST'){res.setHeader('Allow','GET, POST');return http.reply(res,405,{error:'Method not allowed.'})}if(!http.sameOrigin(req))return http.reply(res,403,{error:'Please play from the game website.'});if(!secret)return http.reply(res,503,{error:'Tactical mode is waiting for a stable campaign signing secret.'});let body;try{body=http.parseBody(req)}catch{return http.reply(res,400,{error:'Invalid request.'})}if(rateLimit(req,res))return;const input=validate(body);return input?resolveAction(input,secret,res):http.reply(res,400,{error:'Invalid tactical action.'})};
